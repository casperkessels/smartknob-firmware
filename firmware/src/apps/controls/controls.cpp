...
ClimateControlApp::ClimateControlApp(SemaphoreHandle_t mutex, char *app_id_, char *friendly_name_, char *entity_id_) : App(mutex)
{
...
int8_t ClimateControlApp::navigationNext()
{
    if (mode == ClimateControlAppMode::CLIMATE_TEMP)
    {
        mode = ClimateControlAppMode::FAN_SPEED;
    ...
    else if (mode == ClimateControlAppMode::FAN_SPEED)
    {
        mode = ClimateControlAppMode::SEAT_HEATING;
        lv_obj_add_flag(climate_screen, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(fan_speed_screen, LV_OBJ_FLAG_HIDDEN);
    ...
    else if (mode == ClimateControlAppMode::SEAT_HEATING)
    {
        mode = ClimateControlAppMode::CLIMATE_TEMP;
...
EntityStateUpdate ClimateControlApp::updateStateFromKnob(PB_SmartKnobState state)
{
...
void ClimateControlApp::initFanSpeed()
{
...
void ClimateControlApp::updateFanSpeed()
{
    SemaphoreGuard lock(mutex_);
...
void ClimateControlApp::initSeatHeating()
{
    SemaphoreGuard lock(mutex_);
...
void ClimateControlApp::updateSeatHeating()
{
    SemaphoreGuard lock(mutex_);
...
void ClimateControlApp::initScreen()
{
    {
        SemaphoreGuard lock(mutex_);
...
void ClimateControlApp::initTemperatureArc()
{
    {
        SemaphoreGuard lock(mutex_);
...
void ClimateControlApp::updateTemperatureArc()
{
    {
        SemaphoreGuard lock(mutex_);
...
void ClimateControlApp::updateModeIcon()
{
    SemaphoreGuard lock(mutex_);
    ...
    switch (mode)
    {
    case ClimateControlAppMode::CLIMATE_TEMP:
    ...
    case ClimateControlAppMode::FAN_SPEED:
    ...
    case ClimateControlAppMode::SEAT_HEATING:
...
void ClimateControlApp::updateStateFromHASS(MQTTStateUpdate mqtt_state_update)
{
    ...
    if (mode != NULL)
    {
        if (mode->valueint >= 0 && mode->valueint < ClimateControlAppMode::MODE_COUNT)
        {
            this->mode = static_cast<ClimateControlAppMode>(mode->valueint);
        }
        else
        {
