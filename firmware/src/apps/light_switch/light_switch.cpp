...
void LightSwitchApp::initScreen()
{
    ...
    lv_arc_set_rotation(arc_, 225);    // start at 225 degrees
    lv_arc_set_bg_angles(arc_, 0, 90); // 90 degree total arc
...
EntityStateUpdate LightSwitchApp::updateStateFromKnob(PB_SmartKnobState state)
{
    ...
    // Calculate arc position based on current position
    // We have 90 degrees total arc divided into 4 positions (0,1,2,3)
    // So each position gets 30 degrees
    uint8_t arc_value = (current_position * 100) / 3; // Convert 0-3 to 0-100%

    {
        ...
        // Update other UI elements based on position
        ...
        else
        {
            ...
            uint8_t brightness = (current_position * 85); // 0, 85, 170, or 255
            lv_obj_set_style_bg_color(screen, LV_COLOR_MAKE(brightness / 3, brightness / 3, 0), 0);
            lv_obj_set_style_arc_color(arc_, lv_color_mix(dark_arc_bg, LV_COLOR_MAKE(brightness / 3, brightness / 3, 0), 128), LV_PART_MAIN);
        }
    }
